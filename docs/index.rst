...
More broadly, it supports a wide range of :ref:`use cases <Use cases>` for
stateless, token-based authentication.

.. admonition:: Please review :ref:`(in)security considerations <(In)security>` before using django-sesame.
    :class: warning

    Also, please don't use libraries published by strangers on the Internet
...
- Django 4.2 (LTS), 5.0, 5.1, and 5.2 (LTS);
- Python ≥ 3.9.
...
You can head over to the :ref:`tutorial <Tutorial>` for complete examples or
continue reading for a shorter demo.

Quick example
-------------

Configure :class:`~sesame.views.LoginView` in your URLconf:
...
1. :ref:`Login by email`, an attractive option on mobile where typing passwords
   is uncomfortable. This technique is prominently deployed by Slack.

   If you're doing this, you should define a small :data:`SESAME_MAX_AGE`,
   perhaps 10 minutes.

2. :ref:`Authenticated links`. For example, you can generate a report offline
   ...
   Likewise, you should configure an appropriate :data:`SESAME_MAX_AGE`,
   probably a few days.
...
creating :ref:`short-lived <Tokens expiration>` or :ref:`single-use <Single-use
tokens>` tokens.

Otherwise, a reasonable attempt was made to provide a :ref:`secure solution
<Tokens design>`. Tokens are secured with modern cryptography. There are
configurable options for :ref:`token invalidation <Tokens invalidation>`.
